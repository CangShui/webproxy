...
function isHostLike(h){h=String(h||"").toLowerCase();if(!h)return false;var i=h.lastIndexOf(":");if(i>0){var port=h.slice(i+1);if(!/^\d+$/.test(port))return false;h=h.slice(0,i);}if(!h||h.charAt(0)==="."||h.charAt(h.length-1)==="."||h.charAt(0)==="-"||h.charAt(h.length-1)==="-")return false;if(h.indexOf(".")<0)return false;if(!/^[a-z0-9.\-]+$/.test(h))return false;var last=h.slice(h.lastIndexOf(".")+1);if(last.length<2||last.length>24)return false;if(!/^[a-z]+$/.test(last))return false;if(/^(a?png|aspx?|avif|bmp|cgi|cfm|css|csv|do|docx?|eot|gif|gz|htm[l]?|ico|jpe?g|jsp|js|json|map|md|mjs|cjs|mov|mp3|mp4|otf|pdf|php|pl|png|ppt[x]?|py|rar|rss|shtml|svg|tar|tex|ts|ttf|txt|wav|wasm|webmanifest|webm|webp|woff2?|xlsx?|xml|zip|data|_data|loader|__loader)$/.test(last))return false;return true;}
function isHostPrefix(h){return isKnown(h)||isHostLike(h);}
function isPageHost(h){return isHostPrefix(h)&&!isDirect(h)&&h!==ownHost();}
function isUnroutable(h){h=String(h||"").toLowerCase();var i=h.indexOf(":");if(i>0)h=h.slice(0,i);if(!h||h==="localhost")return true;if(/^127\./.test(h))return true;if(/^::1$/.test(h))return true;if(h.indexOf(".")<0)return true;return false;}function isProxy(h){h=String(h||"").toLowerCase();var i=h.indexOf(":");if(i>0)h=h.slice(0,i);if(!h||h===ownHost())return false;if(isDirect(h))return false;if(isUnroutable(h)&&!isKnown(h))return false;return true;}
...
function proxify(u){if(typeof u!=="string"||u==="")return u;try{var a=new URL(u,base());if(a.protocol!=="http:"&&a.protocol!=="https:")return u;if(isProxy(a.hostname)){if(ROOT&&a.hostname===TARGET){return rewriteParams(OWN+a.pathname+a.search+a.hash);}return rewriteParams(OWN+"/"+a.host+a.pathname+a.search+a.hash);}var oh=ownHost();if(oh&&a.hostname===oh){var pth=a.pathname||"";if(pth.indexOf("/backend-api/sentinel/")===0){return rewriteParams(u);}var seg=a.pathname.indexOf("/",1);var first=seg<0?a.pathname.slice(1):a.pathname.slice(1,seg);if(first==="http:"||first==="https:"){try{var eu=new URL(pth.slice(1),base());eu.pathname=eu.pathname.replace(/\/{2,}/g,"/");return proxify(eu.href);}catch(e){return rewriteParams(u);}}if(first===""||!isPageHost(first)){var ph=pageHost();if(ph){return rewriteParams(OWN+"/"+ph+(pth===""?"/":pth)+a.search+a.hash);}}return rewriteParams(u);}return rewriteParams(u);}catch(e){return u;}}
...
try{var _rrv=null;Object.defineProperty(window,"__reactRouterManifest",{configurable:true,get:function(){return _rrv;},set:function(v){if(v&&typeof v==="object"){try{var rrPh=pageHost();function rrP(pp){if(typeof pp!=="string"||pp===""||pp.charAt(0)!=="/")return pp;var rrI=pp.indexOf("/",1);var rrF=rrI<0?pp.slice(1):pp.slice(1,rrI);if(isPageHost(rrF))return pp;if(rrPh)return "/"+rrPh+pp;return pp;}function rrW(o){if(!o||typeof o!=="object")return;if(typeof o.module==="string")o.module=rrP(o.module);if(Array.isArray(o.css)){for(var ci=0;ci<o.css.length;ci++)o.css[ci]=rrP(o.css[ci]);}if(Array.isArray(o.imports)){for(var ii=0;ii<o.imports.length;ii++)o.imports[ii]=rrP(o.imports[ii]);}for(var rk in o){if(Object.prototype.hasOwnProperty.call(o,rk)){var rv=o[rk];if(rv&&typeof rv==="object")rrW(rv);}}}rrW(v);}catch(e){}}_rrv=v;}});}catch(e){}
function proxifyHist(u){if(typeof u!=="string"||u==="")return u;try{var a=new URL(u,location.href);if(a.protocol!=="http:"&&a.protocol!=="https:")return u;if(isProxy(a.hostname)){if(ROOT&&a.hostname===TARGET){return rewriteParams(OWN+a.pathname+a.search+a.hash);}return rewriteParams(OWN+"/"+a.host+a.pathname+a.search+a.hash);}var oh=ownHost();if(oh&&a.hostname===oh){var pth=a.pathname||"/";if(pth.indexOf("/backend-api/sentinel/")===0){return rewriteParams(a.href);}var seg=pth.indexOf("/",1);var first=seg<0?pth.slice(1):pth.slice(1,seg);if(first!==""&&isPageHost(first)){return rewriteParams(a.href);}var ph=pageHost();if(ph){return rewriteParams(OWN+"/"+ph+pth+a.search+a.hash);}return rewriteParams(a.href);}return rewriteParams(u);}catch(e){return u;}}
try{var h=window.history;var hps=h.pushState;var hrs=h.replaceState;if(hps){h.pushState=function(s,t,u){if(typeof u==="string"){var p=proxifyHist(u);if(crossHostNav(p)){window.location.href=p;return;}if(p!==u)arguments[2]=p;}return hps.apply(this,arguments);};}if(hrs){h.replaceState=function(s,t,u){if(typeof u==="string"){var p=proxifyHist(u);if(crossHostNav(p)){window.location.href=p;return;}if(p!==u)arguments[2]=p;}return hrs.apply(this,arguments);};}}catch(e){}
document.addEventListener("click",function(e){try{var t=e.target;var a=t&&t.closest?t.closest("a[href]"):null;if(!a)return;var h=a.getAttribute("href");if(!h)return;var p=proxify(h);if(p===h)return;a.setAttribute("href",p);}catch(err){}},true);
try{if(AUTH&&!sessionStorage.getItem("__wplogin")){sessionStorage.setItem("__wplogin","1");fetch("/__login",{method:"POST",headers:{Authorization:AUTH},credentials:"include"}).catch(function(){})}}catch(e){}
})();`
