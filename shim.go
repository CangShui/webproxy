...
function proxify(u){if(typeof u!=="string"||u==="")return u;try{var a=new URL(u,base());if(a.protocol!=="http:"&&a.protocol!=="https:")return u;if(isProxy(a.hostname)){return rewriteParams(OWN+"/"+a.host+a.pathname+a.search+a.hash);}var oh=ownHost();if(oh&&a.hostname===oh){var pth=a.pathname||"";if(pth.indexOf("/backend-api/sentinel/")===0){return rewriteParams(u);}var seg=a.pathname.indexOf("/",1);var first=seg<0?a.pathname.slice(1):a.pathname.slice(1,seg);if(first==="http:"||first==="https:"){try{var eu=new URL(pth.slice(1),base());eu.pathname=eu.pathname.replace(/\/{2,}/g,"/");return proxify(eu.href);}catch(e){return rewriteParams(u);}}if(first!==""&&!isPageHost(first)){var ph=pageHost();if(ph){return rewriteParams(OWN+"/"+ph+pth+a.search+a.hash);}}return rewriteParams(u);}return rewriteParams(u);}catch(e){return u;}}
